...
L'API ML stocke les probabilités de ses 100 dernières prédictions en direct dans une file circulaire en mémoire (Redis ou mémoire cache de l'API).
...
## 🐳 5. Industrialisation & Intégration Docker Compose
...
## 🖥️ 6. L'Interface Frontend "AI Insights" : La Face Visible du MLOps
...
3.  **Moniteur de Drift** :
    *   Un composant visuel (jauge lumineuse) représentant le statut `l1_ml_concept_drift_status`. Si la jauge vire au orange ou au rouge, un message explicatif indique au jury que les conditions du championnat ont changé (ex: mercato) et qu'un réentraînement est nécessaire.
